...
(function () {
    ...
    // INTERNAL API: FETCH SYSTEM STATS (/stats)

    // Returns cpu_load, ram info, uptime from the local Python controller.

    // Returns null if the server is not running.

    // =============================================================================

    async function fetchInternalStats() {

        try {

            const response = await fetch(`${INTERNAL_API_URL}/stats`, {
    ...
    // INTERNAL API: TRIGGER CONTAINER RESTART (/restart)

    // Calls the local Python controller to kill the process and let Heroku restart.

    // =============================================================================

    async function triggerInternalRestart(reason = 'health_server_request') {

        try {

            const response = await fetch(`${INTERNAL_API_URL}/restart`, {
    ...
    // Supported incoming message types from server:
    //   - "report_request" / "ping_health" → run dummy claim, gather all stats, send health_report
    //   - "restart_request"                → call INTERNAL_API_URL/restart, ack back to server
    //
    // Outgoing periodic message (every 30s):
    //   - "token_report" → token count + system stats from INTERNAL_API_URL/stats
    // ================================
    ...
    function connectHealthSocket() {
        ...
        try {
            ...
            healthWsSocket.onopen = () => {
                ...
                // Start periodic token + system stats reporting every 30 seconds
                ...
                healthWsReportInterval = setInterval(async () => {
                    ...
                    if (healthWsSocket && healthWsSocket.readyState === WebSocket.OPEN) {
                        ...
                        // Fetch system stats from internal API alongside token count

                        const systemStats = await fetchInternalStats();
                        ...
                        const report = {
                            ...
                            // Include system stats if available (null if internal server is down)
            ...
            healthWsSocket.onmessage = async (event) => {
                ...
                // ── RESTART REQUEST ──────────────────────────────────────────────────────
                // Health server sends {"type":"restart_request"} to trigger a container restart.
                // We call the internal Python API /restart endpoint and ack the result back.

                if (msg.type === 'restart_request') {

                    const restartReason = msg.reason || 'health_server_request';

                    addLog(`[Health] Restart requested by server (reason: ${restartReason}). Calling internal restart API...`, 'warning');

                    const restartResult = await triggerInternalRestart(restartReason);

                    // Acknowledge back to health server
                    ...
                    try {
                        ...
                        healthWsSocket.send(JSON.stringify({
                            ...
                            // Will be null if internal server is not running
                ...
                // Server sends {"type":"report_request"} or {"type":"ping_health"} to trigger a full report
                ...
                if (msg.type === 'report_request' || msg.type === 'ping_health') {
                    ...
                    // ── Fetch system stats from internal Python API ──
                    ...
                    const report = {
                        ...
                        // System stats from internal Python controller (null if not running)
                    ...
                    try {
                        ...
                        addLog(`[Health] Report sent (latency: ${dummyLatency}ms, tokens: ${tokenCacheCount}, ram: ${systemStats ? systemStats.ram_free_mb + 'MB free' : 'N/A'})`, 'success');
